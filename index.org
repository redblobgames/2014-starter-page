...
- Svelte v4 is similar to v3
- Svelte v5 changes things and I haven't had a chance to try it
...
- Svelte does /simple static analysis/ to find dependencies. See [[https://v4.svelte.dev/docs/svelte-components#script-3-$-marks-a-statement-as-reactive][the =yPlusAValue= example in the documentation]], in which =total= depends on both =x= and =y=. Svelte detects only changes to =x=, not changes to =y=, even though both are reactive. Refactoring code can make Svelte lose track of reactivity. In my example code, I had to use a workaround on =classFor=. [[https://github.com/sveltejs/svelte/issues/5190][Here's another example of refactoring that fails]]. For Svelte 5 they are [[https://svelte.dev/blog/runes#runtime-reactivity][switching to dynamic analysis]].
...
React's system re-runs the component code every time, and then you use things like =useMemo= to prevent that. Vue's system separates the /setup/, which runs once, and the /template/ which runs every time. In d3.js it's best to separate these as well, using =.enter()= / =.exit()=.  See [[https://chrlschn.dev/blog/2025/01/the-inverted-reactivity-model-of-react/][Charles Chen's article on React reactivity]].
...
<x:footer>
  Created 10 Apr 2020;
  <!-- hhmts start -->Last modified: 16 Apr 2025<!-- hhmts end -->
</x:footer>
#+end_export
