...
I've made a partial attempt at [[https://github.com/redblobgames/2014-starter-page/tree/master/svelte-v3][sample code]] but the reactivity doesn't work right, and since I am not planning to use Svelte (because I'm avoiding framework-specific build steps [[href:/making-of/little-things/#site-build-reduce][because it doesn't match my long term goals]]), I have not tried to fix it.
...
- Svelte tracks changes to top level variables based on assignment statements, but not to changes made to /properties/ of objects or /elements/ of arrays. See [[https://svelte.dev/tutorial/updating-arrays-and-objects][their tutorial]] for workarounds.
- If you change variable ~x~, Svelte will automatically redraw a template that uses =x= directly, but /not/ if the template calls ~f()~ and that function returns =x=. It seems to /only/ detect direct uses of =x=.
